...
/**
 * @returns {Promise<{cid:string, provider:'pinata'|'local', size:number, gatewayUrl:string}>}
 */
async function upload(buffer, fileName = "record.enc") {
  ...
  return { cid, provider: "local", size: buffer.length, gatewayUrl: gatewayUrl(cid) };
}
...
module.exports = { upload, fetchBlob, gatewayUrl, computeCid };
